    ...
    int batch_size)
{
    ...
    while(taskIdx < batch_size)
    {
        ...
        float3 doffset_dpts = make_float3(level_resolution-1);
        ...
        // trilinear weights used in backward
        ...
        // gradient of the hashgrid output to the input
        #pragma unroll
        for (int i=0; i<8; i++)
        {
...
// calculating double backward
__global__ void embedding_backward_backward_kernel(    
    float3* points, // B x 3 
    float3* d_grad_points, // dL_dpts  B x 3 
    float2* d_grad_in, // B x (Levels x 2) dL_dfeature_out
    float2* features, // Levels x hashmap_size x 2
    int n_levels, int hashmap_size,
    int3* resolutions, 
    int batch_size)
{
    ...
    while(taskIdx < batch_size)
    {
        ...
        int index = taskIdx * n_levels + blockIdx.y;
        
        atomicAdd(&d_grad_in[index].x, doffset_dpts.x * dout_dx.x * d_grad_points[taskIdx].x);
        atomicAdd(&d_grad_in[index].y, doffset_dpts.x * dout_dx.y * d_grad_points[taskIdx].x);

        atomicAdd(&d_grad_in[index].x, doffset_dpts.y * dout_dy.x * d_grad_points[taskIdx].y);
        atomicAdd(&d_grad_in[index].y, doffset_dpts.y * dout_dy.y * d_grad_points[taskIdx].y);
        
        atomicAdd(&d_grad_in[index].x, doffset_dpts.z * dout_dz.x * d_grad_points[taskIdx].z);
        atomicAdd(&d_grad_in[index].y, doffset_dpts.z * dout_dz.y * d_grad_points[taskIdx].z);
